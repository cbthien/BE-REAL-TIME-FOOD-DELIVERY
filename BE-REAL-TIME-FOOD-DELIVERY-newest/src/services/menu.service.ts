...
export class MenuService {
  ...
  async createMenuItem(createMenuItemDto: CreateMenuItemDto): Promise<MenuItem> {
    ...
    if (createMenuItemDto.images?.length) {
      menuItem.images = this.buildMenuItemImages(menuItem, createMenuItemDto.images);
    }
  ...
  category.description = trimmedDescription.length > 0 ? trimmedDescription : undefined;
  }
  ...
  async updateMenuItem(id: number, dto: UpdateMenuItemDto) {
    ...
    if (dto.categoryId !== undefined && dto.categoryId !== menuItem.categoryId) {
    ...
    if (dto.description !== undefined) {
  const trimmedDescription = dto.description.trim();
  menuItem.description = trimmedDescription.length > 0 ? trimmedDescription : undefined;
    }
